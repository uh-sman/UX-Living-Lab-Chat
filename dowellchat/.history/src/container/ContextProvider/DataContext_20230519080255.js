...
export const AppProvider = ({ children }) => {
  ...
  const [room_Id, setRoom_Id] = useState(3);
  ...
  useEffect(() => {
    const getMessage = async (title) => {
      try {
        const res = await axios.get(
          `https://100096.pythonanywhere.com/room_list/Login${room_Id}`
        );

        console.log("res.data from context", res?.data?.messages);
        setRoom(res?.data?.messages);
      } catch (error) {
        console.error("error", error);
      }
    };
